...
//   … --if-missing                          — ничего не делать, если справочник
//                                             уже загружен (для автозапуска в Docker).
//
...
const ifMissing = !!flag('if-missing');
// Столько кодов в открытой базе после чистки дублей (~1,09 млн). Если в
// справочнике уже больше — считаем, что базу загрузили, и не качаем снова.
// Прерванная загрузка даёт меньше — тогда докачиваем, дубли пропускаются.
const LOADED_MIN = 1_000_000;
...
async function download(url: string): Promise<string> {
  ...
  // Качаем во временный файл: оборванная закачка не должна выглядеть как готовая.
...
async function main() {
  ...
  if (ifMissing) {
    const { rows } = await pool.query(`SELECT to_regclass('barcode_catalog') IS NOT NULL AS has`);
    const count = rows[0].has
      ? (await pool.query(`SELECT count(*)::int AS count FROM barcode_catalog`)).rows[0].count
      : 0;
    if (count >= LOADED_MIN) {
      console.log(`  уже загружен: ${count.toLocaleString('ru')} кодов — пропускаю.`);
      await pool.end();
      return;
    }
  }
